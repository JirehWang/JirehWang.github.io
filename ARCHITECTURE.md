...
## 2026-08 Supabase 熱響應架構更新 (敬拜團 + 教會行事曆系統)

- 敬拜團系統（`LKC_worship`）與教會行事曆系統（`LKC_MasterSchedule`）正式將熱響應後端遷移至 **Supabase (PostgreSQL + PostgREST)**。
- **冷熱分流架構**：
  - **熱資料（Hot Tier，>= 2025 年）**：直連 Supabase，讀寫延遲縮短至 **<100ms**，提供極速月曆翻頁、篩選、事項動態欄位渲染、批次建立、排班與曲目管理。
  - **冷資料（Cold Tier，< 2025 年）**：前端透明 fallback 走原有 GAS 讀取 Google Sheets 歷史存檔。
- 前端透過 `apps/LKC_worship/worship-supabase.js` 與 `apps/LKC_MasterSchedule/calendar-supabase.js` 適配器接管 `callAPI`，UI 畫面與業務邏輯無縫相容。
...
#### 🛠️ 顯示與資料修正
- `getAttendanceTrend` 回傳的每位會友資料補上 `inGroup`，判斷方式與一般統計一致：直接從主日「會友名單」的「所屬小組」欄判斷是否屬於任一小組。
- `apps/LKC_SundayserviceAttendance/STATS.js` 在「出席頻率變化分析」名單卡片的姓名旁顯示綠色 `小組` 標籤，讓關懷同工能一眼辨識該會友是否已有小組。
- 頻率變化分析 CSV 匯出同步加入「小組」欄位，內容以 `是` / `否` 表示。

#### 📁 影響檔案
- 前端：`apps/LKC_SundayserviceAttendance/STATS.js`
- GAS：`主日出席_測試版/ReportService.js`（已透過 `clasp push` 同步）
...
3. **`.gitignore` 預防**
   ...
   - Gemini API Key 集中在「LKC系統設定」試算表的 `AI_Config`，由主 GAS `GeminiHelper.js` 讀取並快取；PrayerPPT 以專用 `LKC_PrayerPPT` 中央路由呼叫合併主 GAS 的 `cal_parsePrayerImage`，前端可批量選取圖片並逐張辨識，每張結果獨立解析後再合併段落，避免跨圖片前言與頁碼污染，且不接觸 key；經文與金句區塊只保留書卷章節代號，前端抽取代號後交既有聖經 API 取得全文。PrayerPPT 與共用版面群組介面採相同的群組建立、頁面歸屬與套用契約；預設白底使用深色標題與內文，清單以大項分頁並在可用空間內合併小點
