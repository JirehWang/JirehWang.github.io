...
3. **`.gitignore` 預防**
   ...
   - Gemini API Key 集中在「LKC系統設定」試算表的 `AI_Config`，由主 GAS `GeminiHelper.js` 讀取並快取；PrayerPPT 以專用 `LKC_PrayerPPT` 中央路由呼叫合併主 GAS 的 `cal_parsePrayerImage`，前端可批量選取圖片並逐張辨識，全部完成後合併匯入，且不接觸 key
