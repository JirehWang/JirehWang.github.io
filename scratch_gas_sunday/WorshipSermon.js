// ==========================================
// 📖 1. 讀取講員資訊 (含 UUID 與區間過濾機制) WorshipSermon.js
// ==========================================
/**
...
function getSermonInfo(payload) {
  ...
  // 若有選取區間，則設定過濾條件
  const startNum = startDate ? worship_toDateNum(startDate) : 0;
  const endNum = endDate ? worship_toDateNum(endDate) : 99999999;

  let result = [];
  const tz = ss.getSpreadsheetTimeZone();

  extData.slice(1).forEach(row => {
    const dNum = worship_toDateNum(row[extDateIdx]);
    ...
    if (dNum >= startNum && dNum <= endNum) {
      ...
      // 🌟 關鍵：用日期+類別去剛才建立 the 字典找找看，是不是已經有講道資料了？
...
function saveSermonInfo(payload) {
  ...
  // 🌟 修正：改用 SpreadsheetTimeZone 確保刪除期限精準（時區提到迴圈外，避免重複呼叫）
  const tz = ss.getSpreadsheetTimeZone();
  const cutoffDateStr = Utilities.formatDate(cutoffDate, tz, "yyyy-MM-dd");
  ...
  dbMap.forEach((obj, uuid) => {
    ...
    if (objDateStr instanceof Date) {
      objDateStr = Utilities.formatDate(objDateStr, tz, "yyyy-MM-dd");
    }
