...
test('keeps announcements, church news, and pastoral prayer in report-page order', () => {
  ...
  assert.match(pages[3].body, /3\. 教界三/);
  assert.equal(pages[4].title, '報告－關懷代禱');
  assert.match(pages[4].body, /在家調養兄姐：王小明/);
  assert.match(pages[4].body, /住院：陳小華/);
  assert.match(pages[4].body, /其他代禱：為社區代禱/);
});
