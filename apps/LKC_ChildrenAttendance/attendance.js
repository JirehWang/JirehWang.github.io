  ...
  function saveNewGroup() {
    let category = document.getElementById('newGroupCategoryCustom').value.trim();
    if (!category) {
      category = document.getElementById('newGroupCategory').value;
    }
    if (!category) return alert("請選擇或輸入類別名稱！");
    ...
    google.script.run
      ...
      .withSuccessHandler(config => {
        ...
        closeGroupAddModal();
        document.getElementById('newGroupCategoryCustom').value = '';
        globalGroupConfig = config;
...
function executeRevoke(uid, displayName) {
  ...
  function downloadVenueJumpCard(cat, grp) {
    ...
    const finalUrl = "https://jirehwang.github.io/LKC1958_June_1.github.io/apps/LKC_ChildrenAttendance/?cat=" + encodeURIComponent(cat) + "&grp=" + encodeURIComponent(grp) + "&v=" + timestamp;
