  ...
  if (sheet) {
    ...
    if (data.length > 1) {
      ...
      data.slice(1).forEach(row => {
        const dNum = toDateNum(row[dateIdx]);
...
function saveSongs(payload) {
      ...
      if (dNum > 0) {
        existingDict[`${dNum}_${mName}`] = {
          '日期': row[dIdx] instanceof Date
            ? Utilities.formatDate(row[dIdx], ss.getSpreadsheetTimeZone(), "yyyy-MM-dd")
  ...
  const cutoffStr = Utilities.formatDate(cutoffDate, ss.getSpreadsheetTimeZone(), "yyyy-MM-dd");

  let finalRows = [];
  Object.values(existingDict).forEach(obj => {
    let dateStr = obj['日期'];
    if (dateStr instanceof Date) dateStr = Utilities.formatDate(dateStr, ss.getSpreadsheetTimeZone(), "yyyy-MM-dd");
    if (String(dateStr).trim() >= cutoffStr) finalRows.push(obj);
  });

  finalRows.sort((a, b) => toDateNum(a['日期']) - toDateNum(b['日期']));
